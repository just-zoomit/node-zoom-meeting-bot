...
SDKError Zoom::config(std::string cmd) {
    auto status = m_config.read(cmd);
    if (status) {
...
SDKError Zoom::init(RawAudioRecordingCallback cb) {
    m_audio_callback = cb;
...
//TODO: remove
    auto participantCtl = m_meetingService->GetMeetingParticipantsController();
    int myUid = participantCtl->GetMySelfUser()->GetUserID();
    std::cout << "myUid " << myUid << " " << participantCtl->GetMySelfUser()->GetUserName() << std::endl;

    auto participantsList = participantCtl->GetParticipantsList();

    for (int i = 0; i < participantsList->GetCount(); ++i) {
        int uid = participantsList->GetItem(i);
        auto name = participantCtl->GetUserByUserID(uid)->GetUserName();
        std::cout << "Item at index " << i << ": " << uid << name << std::endl;
    }
    ...
    if (m_config.useRawAudio()) {
        ...
        if (!m_audioSource) {
            m_audioSource = new ZoomSDKAudioRawDataDelegate(!m_config.separateParticipantAudio(), m_audio_callback);
