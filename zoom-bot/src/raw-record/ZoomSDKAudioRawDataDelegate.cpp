...
ZoomSDKAudioRawDataDelegate::ZoomSDKAudioRawDataDelegate(bool useMixedAudio, RawAudioRecordingCallback cb) : m_useMixedAudio(useMixedAudio)
{
    m_audio_callback = cb;
}
...
void ZoomSDKAudioRawDataDelegate::onShareAudioRawDataReceived(AudioRawData* data) {
    stringstream ss;
    ss << "Shared Audio Raw data: " << data->GetBufferLen() << "b at " << data->GetSampleRate() << "Hz" << data->GetChannelNum() << "ch";
    Log::info(ss.str());
}
...
void ZoomSDKAudioRawDataDelegate::writeToFile(const string &path, AudioRawData *data)
{
    ...
    stringstream ss;
    ss << "Writing " << data->GetBufferLen() << "b to " << path << " at " << data->GetSampleRate() << "Hz" << data->GetChannelNum() << "ch";
